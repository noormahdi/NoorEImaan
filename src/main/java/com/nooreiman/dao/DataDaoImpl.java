...
public class DataDaoImpl implements DataDao {
	...
	public Ayat getAyatById(long id) throws Exception {
		session = sessionFactory.openSession();
		Ayat ayat = (Ayat) session.load(Ayat.class, new Long(id));
	...
	public Surah getSurahById(long id) throws Exception {
		session = sessionFactory.openSession();
		Surah surah = (Surah) session.load(Surah.class, new Long(id));
		tx = session.getTransaction();
		session.beginTransaction();
		tx.commit();
		return surah;
	...
	public List<Ayat> getAyatList() throws Exception {
		session = sessionFactory.openSession();
		tx = session.beginTransaction();
		List<Ayat> ayatList = session.createCriteria(Ayat.class).list();
		tx.commit();
		session.close();
		return ayatList;
	}
	...
	public List<Translation> getTranslationList() throws Exception {
		session = sessionFactory.openSession();
		tx = session.beginTransaction();
		List<Translation> translationList = session.createCriteria(Translation.class).list();
		tx.commit();
		session.close();
	...
	public Translation getTranslationById(long id) throws Exception {
		session = sessionFactory.openSession();
		Translation translation = (Translation) session.load(Translation.class, new Long(id));
		tx = session.getTransaction();
		session.beginTransaction();
		tx.commit();
		return translation;
